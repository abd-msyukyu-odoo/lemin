...
int				main(void)
{
	...
	//display_hmap_repartition(&g_lemin->hm_rooms);
	display_hm(&g_lemin->hm_rooms, show_room);
	display_start(g_lemin->start);
	display_end(g_lemin->end);
	display_n_ants(g_lemin->n_ants);
	global_free();
	end = clock();
